															...
															foreach($all_visitor as $data_visitor){ ?>
																<tr>
																	...
																	<!-- Membuat sebuah textbox hidden yang akan digunakan untuk detail visitor -->
																		...
																		<input type="hidden" class="dvioa-value_data" value="<table class='table text-center table-responsive-sm'><thead><tr><th colspan='2'>Waktu Berkunjung ke Area</th></tr></thead><tbody><tr><td>Waktu Masuk ke Area <br><b><?= date('D, d-M-Y H:i:s', strtotime($data_tracking->time_in_area)); ?></b></span></td><td>Waktu Keluar dari Area <br> <b><?php if($data_tracking->time_out_area != null){ echo date('D, d-M-Y H:i:s', strtotime($data_tracking->time_out_area));}else{echo "<i class='text-danger'>belum scan keluar area</i>";} ?></b></span></td></tr><tr><td colspan='2'>Lama Berkunjung ke Area <br> <b><?php if($data_tracking->time_out_area != null){ $get_waktu_berkunjung_area = $this->db->query("SELECT SUM(TIMEDIFF(time_out_area,time_in_area)) as 'total_lama_berkunjung_area' FROM tabel_tracking WHERE id_visitor='".$data_visitor->id_visitor."' AND id_area='".$data_tracking->id_area."'")->row("total_lama_berkunjung_area"); echo floor($get_waktu_berkunjung_area / 3600).' jam '.floor(($get_waktu_berkunjung_area / 60) % 60). ' menit '.floor($get_waktu_berkunjung_area % 60).' detik';;}else{echo "<i class='text-danger'>belum scan keluar area</i>";} ?></b></td></tr></tbody></table>">
																</tr>
															<?php }
														}
													?>
					...
					<tbody>
						<?php foreach($all_visitor_join as $data_visitor){ ?>
							<tr>
								...
								<td><?php if($data_visitor->time_out_event != null){ echo date('D, d-M-Y H:i:s', strtotime($data_visitor->time_out_event)); }else{ echo "<i class='text-danger'>belum scan keluar event</i>"; } ?></td>
								<td><a href="javascript:void()" data-id="<?= $data_visitor->id_visitor; ?>" data-toggle="modal" data-target="#modal_detail_visitor" class="btn btn-info btn-detail-visitor m-1">Detail</a></td>
